...
import { parseViolationRules } from '../src/moderation/rules'

describe('parseViolationRules', () => {
  it('parses [variants] lines into a canonical map', () => {
    const { variants, skipped } = parseViolationRules(
      '# c\n[variants]\n微信=薇信|威心\n刷单=刷dan,唰单\nQQ=扣扣\n',
    )
  ...
  it('rejects malformed [variants] lines', () => {
    const { variants, skipped } = parseViolationRules('[variants]\n=xxx\n微信=\n微信=薇\nQQ=扣扣\n')
    assert.deepEqual(variants.QQ, ['扣扣'])
  ...
  it('unknown sections are reported and their lines skipped', () => {
    const { variants, skipped } = parseViolationRules('[foo]\n微信=薇信\n[variants]\nQQ=扣扣\n')
